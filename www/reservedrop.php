...
//	if (Auth::isAdmin() || $cur_user->get_isadmin())
	...
	if ($fn == 'create' || $fn == 'modify' )
	{
		...
		$res->reservation_status = $_POST['reservation_status'];
		
		$res->contractsoort = $_POST['contractsoort'];		
		$res->clusterid = $_POST['clusterid'];		
		$timestamp = date(U);
	...
	if ($fn == 'create') 
	{
		...
		$res->repeat = $repeat;
		$res->add_res($users_to_invite, $resources_to_add); // janr 
		
		print ("<br><input type=\"button\" class=\"button\" onclick=\"reserve('c','','','{$res->id}');\" value=\"artikel toevoegen\"> &nbsp;&nbsp;aan deze reservering, met zelfde gebruiker, zelfde begin en eindtijd"); // janr 2art button aanbieden voor meer artikelenn
		echo ('<br><br>&nbsp;&nbsp;&nbsp;<input type="button" name="btnPrint" value="' . translate('Print Contract') . '" class="button" onclick="selectAllOptions(this);"/>'
	...
	else if ($fn == 'cluster')   		// dit gaat nwe machid toevoegen aan cluster
	{
		...
		if ($_REQUEST['machid1']) {
			...
			if ($newres->check_res(array())) {
				//print "in".$_REQUEST['machid1']; //testing
				$newid = $newres->db->add_res($newres,null,array(),array(),null);
				echo ('Deze '.$_REQUEST['machid1'].' is wel toegevoegd');
			}else {
				echo ('Deze '.$_REQUEST['machid1'].' is NIET toegevoegd');
				//$newres->print_res();
			}
		}
		// nu nog accesoires updaten van master
		$res->mod_res(array(), array(), array(), $resources_to_add, $resources_to_remove, false, false);
		
		
		// print res overzicht
			
	begin_print_resource_data($rs, 1);		
		$clusters = $res->db->get_reservation_clusterdata($res->clusterid);
		print_resource_data_one_resource($res->resource->properties, $res->type, (bool)$res->get_pending(), $conf['app']['allowed_reservation_status'], $res->reservation_status);	
		print_rest_cluster ($res->id ,$clusters);
	end_print_resource_data($rs, 1);		
...
?>
		...
		$newres->id = $_REQUEST['resid'];
		$newres->load_by_id();
		echo ('This is machid1: '.$_REQUEST['machid1']);
		if ($_REQUEST['machid1']) {
			$newres->resource = new Resource($_REQUEST['machid1']);
			echo ('Checkres starting on '.$_REQUEST['machid1']);
			if ($newres->check_res(array())) {
				//print "in".$_REQUEST['machid1']; //testing
				$newid = $newres->db->add_res($newres,null,array(),array(),null);
				echo ('Deze '.$_REQUEST['machid1'].' is WEL toegevoegd');
				...
				echo ('Deze '.$_REQUEST['machid1'].' is NIET toegevoegd');
				$warning = ('Note: '.$newres->resource->properties['name'].' is NIET toegevoegd');
			}
