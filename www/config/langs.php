...
function get_browser_lang() {
	...
	if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) && !empty($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
		...
		$http_accepted = preg_split('/,/',$_SERVER['HTTP_ACCEPT_LANGUAGE']);
		for ($i = 0; $i < count($http_accepted); $i++) {
			foreach ($languages as $lang => $vals) {
				// if (eregi($vals[0], $http_accepted[$i]))
				if (preg_match('/'.$vals[0].'/i', $http_accepted[$i])) // janr
					return $lang;
			}
		}
	}
...
function translate_status_res($num) {
// voor reservation_status vertaling
	global $strings;
	$return = 'onbekend';
	...
	if ($num > -1 ||$num < 6) {
		switch ($num) {
		case 0:
			return $strings['reservation_status0'];
			break;
		case 1:
			return $strings['reservation_status1'];
			break;
		case 2:
			return $strings['reservation_status2'];
			break;
		case 3:
			return $strings['reservation_status3'];
			break;
		case 4:
			return $strings['reservation_status4'];
			break;
		}	
	}
}
...
*/
function translate_emailxxx($email_index) {
	global $email;
	...
	if (func_num_args() <= 1) {
		...
		$sprintf_args = substr($sprintf_args, 0, strlen($sprintf_args) - 1);
		//var_dump ($sprintf_args);
		
		//var_dump ($email[$email_index]);
