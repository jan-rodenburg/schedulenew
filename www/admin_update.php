...
if (!validate_function($user, $fn)) {
    CmnFns::do_error_box(translate('23'.'This is only accessable to the administrator') . '<br />'
...
if (!isset($tools[$fn]) && !isset($tools[$fn])) {		// Validate tool
	CmnFns::do_error_box(translate('Could not determine tool')
		.$tools[$fn]. '<br/><a href="ctrlpnl.php">' . translate('Back to My Control Panel') . '</a>');
	die();
}
else {
	if (isset($tools[$fn])) {
...
* @param none
* janr add_resource admin change
* Het is nu mogelijk om ook de sleutel (=barcode) van een resource in te geven)
* Extra chack: de opgegeven barcode mag nog niet bestaanarcode toevoegen aan de resource Object
* B
* Wijz: als lege barcode dan autoassign unique-key ()zit er al in)
* Wijz : als barcode opgegeven dan -chack: de opgegeven barcode mag nog niet bestaan- do barcode is key
*/
function add_resource() {
	...
	$newmachid = $resource['newmachid']; // barkode


	if ( $newmachid!='') { //janr newmachid is set for add_resource
			
			$resource['machid'] = $newmachid; // barkode=key
			$id = $db->add_resource_barkode($resource); // barcode resource
			//echo ('using key '.$resource['machid'] );
			//echo ('<br>');
	} else {
			$id = $db->add_resource($resource); // standard
	}

	if (isset($resource['autoassign']))		// Automatically give all users permission to reserve this resource
		$db->autoassign($id);

	CmnFns::write_log('Resource added. ' . $resource['name'], $_SESSION['sessionID'], $_SESSION['sessionScheduleAdmin'], $resource);
	print_success();
}
...
function edit_resource() {
	...
	if (( $newmachid!='')  && ( $newmachid!=$resource['machid'])) { //janr newmachid is set for add_resource
			$oldmachid = $resource['machid'];
			$resource['machid'] = $newmachid; // barkode=key
// janr exists in check_
			
						
			$id = $db->add_resource_barkode($resource); // barcode resource janrhier barkode add (indien dubbel dan crash hier
			
			$id = $db->del_one_resource($oldmachid); // barcode resource
			
			$id = $db->update_reservations($oldmachid,$newmachid); // update reservations janrTODO

			
			// update perms and reservations where machid update newmachid
			//$id = $db->update_reservations_resource_barkode($resource)
			//$id = $db->update_perms_resource_barkode($resource);
	} else {
			$id = $db->edit_resource($resource); // standard
	}
	...
	CmnFns::write_log('Resource edited. ' . $resource['name'] . ' ' . $resource['machid'], $_SESSION['sessionID'], $_SESSION['sessionScheduleAdmin'], $resource);
	print_success();
}
...
* @param none
*/
function dup_resource() {

	global $db;
	global $conf;
				// janr
	$resource = $db->get_resource_data($_GET['machid']);

	$db->dup_resource($resource);		// hier moet iets gebeuren met scan barkode

	//    if (isset($resource['autoassign']))		// Automatically give all users permission to reserve this resource
	//	$db->autoassign($id);

	CmnFns::write_log('Resource duplicated. ' . $resource['name'] . 'Copy', $_SESSION['sessionID'], $_SESSION['sessionScheduleAdmin'], $resource);
	print_success();
// end dup
}
...
function add_additional_resource() {
	...
	$id = $db->add_additional_resource( $data['name'], $data['scheduleid'],  $data['number_available'] );
	print_success();
}
...
function edit_additional_resource() {
	...
	$data = check_additional_resource_data(CmnFns::cleanPostVals());
	$db->edit_additional_resource($data['resourceid'], $data['name'], $data['scheduleid'],  $data['number_available'] );
	print_success();
}
...
//print_r ($data);
	...
	$rs['notes']	= $data['notes'];
	$rs['package']	= $data['package'];	
	$rs['description']	= $data['description'];	
	$rs['maintenance']	= $data['maintenance'];	
	$rs['rowner']	= $data['rowner'];	
	$rs['vast_toebehoren']	= $data['vast_toebehoren'];	
	$rs['serial_nr']	= $data['serial_nr'];	
	$rs['aankoopbedrag']	= $data['aankoopbedrag'];
	$rs['datum_aankoop']	= $data['datum_aankoop'];
	$rs['uitleennivo']	= $data['uitleennivo'];
	$rs['waardecat']	= $data['waardecat'];
	$rs['uitleenperiode']	= $data['uitleenperiode'];
	...
	if ( ($data['newmachid']!='') && ($data['newmachid'] != $data['machid'])) { //janr newmachid is set
		$rs['newmachid'] = $data['newmachid'];
		// $empty = $db->check_exists_resourcexxx($data['newmachid']); // if resource exists, then barcode not unique
		//echo $err;
		//if !($empty) { 
		//	$msg[] = translate('Given barcode already exists');
		//}
	}
...
function check_additional_resource_data($data) {
	...
	if (empty($_SESSION['sessionScheduleAdmin'])) // addadditional scheduleid
		array_push($msg, translate('Unclear of what schedule your are admin of...'));
	else
		$rs['scheduleid'] = $_SESSION['sessionScheduleAdmin'];
