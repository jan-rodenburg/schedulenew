...
/**
* Janr
* This file provides output functions for copycluster.php
* based on:
*
*
*
...
//print_r ('<br>basic panel template ' . $clusters);
	...
	$res->reservation_status = "0"; // jarocc
	// jarocc hier heb ik alle data voor save master
	print_resource_data_one_resource($rs, $res->type, $res->get_pending(), $allowed_reservation_status, $res->reservation_status, $res->id);		
	// janrres_status
...
<iframe style="display:none; position:fixed" id="cbframe" width="420" height="600" frameborder="0" src=""></iframe>
			<!-- voor verlenging, biedt delete funktie -->
			<script type="text/javascript">

			
			function deleteReservationFromReserve(resid) {
				if (confirm('Dit verwijderd de cluster copy. Weet je zeker?')) {
					jQuery('#cbframe').show().attr('src','delete-reservation-from-reserve.php?resid='+resid);
					alert("before close: "+window.name);
					//window.close();
				}
				void(0);   
			}
...
* @param object $res Reservation object to work with
*/
function print_buttons_and_hidden(&$res) { //jaro
?>
...
<?php
	...
	echo '<p>';	
		//echo ( ' ownername:'  . $res->user->get_name() );	// testing
		//echo ( ' type-res:' . $type);	// testing
		//echo ( ' newid:' . $res->newid);	// testing
	echo ( '<span class="valop">Stap1:</span>Kies nieuwe User (eventueel). <span class="valop">Stap2:</span>Klik: Save as Copy.<br><span class="valop">Stap3:</span>Nu begin- en eindtijd aanpassen.</p>');	// testing
...
// de buttons
            ...
            echo '<input type="submit"  name="btnSubmit" value="Save as Copy" class="button" onclick="selectAllOptions(this);"/>'
				. '<input type="hidden" name="fn" value="clustercopy" />';
			echo '<input type="hidden" name="newid" value="'.$res->newid.'" />';
			
				
			
			echo ('<input type="button" name="noname" value="' . translate('Cancel and Delete Copy') . '" class="button" ');
			echo ("onclick=\"window.open('delete-reservation-from-reserve.php?resid={$res->newid}&action=deleteandclose' , 'reserve');\"/>");
			
			//echo ('<input type="button" name="noname" value="' . translate('Summary this Copy') . '" class="button" ');
			//echo ("onclick=\"window.open('admin.php?tool=reservations&search={$res->newid}' , 'schedule');\"/>");
...
<table width="100%" border="0" cellspacing="0" cellpadding="1">
  <tr class="tableBorder">
    <td>
      <table class="responsive-one-item reserve begin_print_resource_data" width="100%" border="1" cellspacing="1" cellpadding="0">
<?php
}
/**
...
// view mode - dit is hoe student het ziet.
			...
			echo translate_status_res($clusters[$i]['0'])	; // jaro
...
// testing...
		if (false) {
				?>		
				<td class="cellColor">xx-reservation_status: 
					<?php echo $clusters[$i]['reservation_status']; ?>
				</td>
				<?php
			}
			echo ('</tr>');
...
function print_resource_data_one_resource(&$rs, $type, $pending, $allowed_reservation_status, $allowed_reservation_status_cv, $resid) {
	...
	$allowed_reservation_status_cv =0; // jaro reset for copyclus
	?>
...
?>
    <table class="responsive-one-item reserve print_time_info" width="100%" border="1" cellspacing="0" cellpadding="1">
     <tr class="tableBorder">
      <td>
       <table width="100%" border="0" cellspacing="1" cellpadding="0">
...
?>
   <table width="100%" border="0" cellspacing="0" cellpadding="1">
    <tr class="tableBorder">
     <td>
      <table class="responsive-one-item reserve print_user_info" width="100%" border="1" cellspacing="1" cellpadding="0">
       <tr>
		...
		// jarocc, must first choose new user		
		if (Auth::isAdmin() || true) { echo "<span class=\"button\"><a href=\"javascript:window.open('user_select.php','selectuser','scrollbars=1','height=430,width=570,resizable');void(0);\">" . translate('Change') . '</a></span>'; } 
		?>	
		<?php
		...
		<?php } ?>
		  ...
		  <div id="studentsh" style="display: <?php echo getShowHide('studentsh') ?>">
		       <table class="responsive-one-item reserve print_more_user_info" width="100%" border="1" cellspacing="1" cellpadding="0">
			  <tr>
