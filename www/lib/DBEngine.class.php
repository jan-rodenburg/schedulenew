...
{
    // janr: hier zit iets afhankelijk van welkse server...
	//include_once('DB.php');
...
//echo $db."  =db79<br>";
  ...
  /**
    ...
    * @return fully qualified table name in form: database.table
    */
    function get_table_for_export($table) 
    {
        global $conf;
        return $table;
        //return $this->table_to_database[$table] . '.' . $table;
    ...
    */
    function get_table_data($table, $fields = array('*'), $orders = array(), $limit = NULL, $offset = NULL, $where_clause = NULL, $where_values = array()) {
		...
		// bij user select is sql goed maar volgorde in scherm gek
		// SELECT * FROM db01_login ORDER BY lname ASC,fname,email,klas,osiris_ok_now,demerit_punt
        //print ('get_tabel_data query : ');
		//print ('<div >' . $query . '</div>'); //test
		//print ('<br>');
    ...
    function check_for_error($result) {
        if (DB::isError($result)) {
            CmnFns::do_error_box(translate('There was an error executing your query') . '<br />'
				...
				. '<br />' . '<a href="javascript: history.back();">' . translate('Back') . '</a><br>');
            //    . '<br />' . 'Een dubbele sleutel in de database is opgetreden.<br> Je kunt de code hierboven knippen/plakken en per mail sturen aan <a 
			// href="mailto:post@larka.nl?SUBJECT=[uitleen] Schedule bug - duplicate key">post@larka.nl</a> . (jan rodenburg - webmaster) ');
