...
include_once($basedir . '/lib/db/AdminDB.class.php'); // add door janr yyyyyy
...
class Schedule {
    ...
    var $blackouts = array();
    var $db;
    var $user; // user used to view this schedule (anonymous or currentuser)
	var $currentuser; // user as logged in (added,pike)
    var $scheduleType;
    var $scheduleid;
    var $viewdays;
    var $startDay;
    ...
    */
    function Schedule($scheduleid, $scheduleType = ALL) {
		...
		if ($scheduleid == null) {
			// print ('<br>$scheduleid == null ');
			// janr filter
			if ($user->is_schedule_admin()) {  
					$scheduleid = $_SESSION['sessionScheduleAdmin'];
					$this->scheduleid = $scheduleid;
					// print ('<br>$this->scheduleid  ' . $this->scheduleid); // janr query filter
			} else {
			// janr
    ...
    */
    function print_schedule() {
        global $conf;
			...
			// janr afvangen today janr only today show
			    if (isset($_GET['today']))
					$today = $_GET['today'];
				else
				$today = null;
        ...
        //$user = new User(Auth::getCurrentID()); 
		$user = &$this->currentuser; // pike optimi
        $schedule_list = $this->db->get_schedule_list();
		if ($this->scheduleType == BLACKOUT_ONLY && !$user->is_admin) {  // gewone user!
			$user_schedules = array();
			foreach ($schedule_list as $row) {
				if ($user->is_schedule_admin($row['scheduleid'])) {  //org
		...
		// janr : case of admin  // janr query filter
			//	print ('<br>sessionScheduleAdmin  ' . $_SESSION['sessionScheduleAdmin']); // janr query filter
			//	print ('<br>sessionAdmin  ' . $_SESSION['sessionAdmin']); // janr query filter
		if ($user->is_schedule_admin()) {  
			foreach ($schedule_list as $row) {
				if (($row['scheduleid'] == $_SESSION['sessionScheduleAdmin'])) {
					$this->scheduleid = $row['scheduleid'];
					// print ('<br>$this->scheduleid  ' . $this->scheduleid); // janr query filter
					print_date_span($this->_date, null, $today);
					print_schedule_name($schedule_list, $this->scheduleid); // dit is de schedule van deze admin janr 
				} 
			}
		}
		if (!$user->is_schedule_admin()) {
		    print_date_span($this->_date, $this->title, $today);
			print_schedule_list($schedule_list, $this->scheduleid); // dit is de dropdown keuze uit schedule
		}


		if ($user->is_schedule_admin($this->scheduleid) || $user->is_admin || $this->scheduleType != BLACKOUT_ONLY ) {
		//  janr dit zijn de 3 mini calenders uit schedule.php en blackout.php
	    //   $this->print_calendars();
			...
			// is er filter op artikel
			    if (isset($_GET['machid']))
					$artfilter = trim($_GET['machid']); // deze filter
				else
				$artfilter = null;
			
			$showmoreday=true; // janr only today show
	        // for ($dayCount = 0; ($dayCount < $this->viewdays); $dayCount++) {
			for ($dayCount = 0; (($dayCount < $this->viewdays) && $showmoreday); $dayCount++) {
				...
				$this->print_reservations($artfilter);    // Print reservations for this day
	            end_day_table();                // End the table for this day
				if ($today==1) $showmoreday=false; //  janrtoday
	        }
	        print_summary_div();			
		}
		else {
...
//        print_summary_div();
	...
	*
	* janr : ook een admin kan NIET reservering maken op een NON ACTIVE resource 
	*/
    function canShowReservation($viewable_date, $current_record) {
//		if (Auth::isAdmin()) {
// active // not active
// var_dump ($current_record); goed
		//$user = new User(Auth::getCurrentId()); -- pike using $this->user  instead
    	//if ((Auth::isAdmin() || $user->is_schedule_admin($this->scheduleid)) ) {
    	if ((Auth::isAdmin() || $this->user->is_schedule_admin($this->scheduleid)) && ($current_record['status'] == 'a') ) {
			
			return true; // ook een admin kan NIET reservering maken op een NON ACTIVE resource 
		}

		$is_active = ($current_record['status'] == 'a');  //active
		$has_permission = $this->user->has_perm($current_record['machid']);
		
		return ( $viewable_date && $is_active && $has_permission ); //bool
    }
    ...
    * @param none
    */
    function print_reservations($artfilter=false) {
        global $conf;
		// echo ('artikelfilter' . $artfilter);
        if (!$this->machids)
            return;
        $current_date = $this->_date['current'];        // Store current_date so we dont have to access the array every time
// JANR TEST
		// print ('print_reservations current_date '.$current_date); // is timestamp janr only today show
        ...
        for ($count = 0; $count < count($this->machids); $count++) {
			...
			//echo ($status); //janr end
			$shown = $this->canShowReservation($viewable_date, $cur_resource); //xxxxxxxxxxx
					//		echo ($status); //janr end
					//		echo ($shown); //janr end fout
			...
			// janr add: gehele rij wordt niet getoond indien artikel (status<>'a')

			if ($shown) 
				{
				// echo ('<br>$2  ' . $id);
				// artfilter = dit is de machid waarde ingegeven in 'Search to Timeslot'
				// if (isset($artfilter)) echo ('<br>$artfilter  ' . $artfilter);
				// janr artfilter: tonen als artfilter uit staat of artfilter staat aan en we hebben de juiste resource
				if (!(isset($artfilter)) || ($artfilter == '') || (isset($artfilter) && ($artfilter == $id)))
					{				
...
//		$is_private = $conf['app']['privacyMode'] && !Auth::isAdmin();
		$is_private = $conf['app']['privacyMode'] && ! (Auth::isAdmin() || in_array($this->scheduleid, $user->get_administrated_schedules()));
		...
		// janr	toon ook:
		//	- meer art op deze res?
		//	- reservation_status
		$reservation_status_text = translate_status_res($rs['reservation_status']);
		//$extendsummarytext = "{$rs['summary']} {$reservation_status_text}";
		$extendsummarytext = ( $reservation_status_text ."<br>" . $rs['summary'] );
		
		$summaryhover = new Summary($extendsummarytext); // janr text toekenning aan de summary		
		$summary = new Summary($rs['summary']); 
		if ((bool)$conf['app']['prefixNameOnSummary']) {
			$summary->memberid = "{$rs['memberid']}"; //janr
			$summary->user_name = "{$rs['fname']} {$rs['lname']}";
		    $summaryhover->user_name = "{$rs['fname']} {$rs['lname']}";
		}
        ...
        $mod_view = ( ($is_mine && $viewable_date) || Auth::isAdmin() || in_array($this->scheduleid, $user->get_administrated_schedules())) ? 'm' : 'v';    // To use in javascript edit/view box
		...
		$summaryhover->visible = $showsummary;
					// janr, afhandeling van de user (      codename: userlogin     )
					// user mag eigen res aanklikken en res voor hem gemaakt door admin. dus alles uitzetten
				 		
		
		if (!$is_mine && !(Auth::isAdmin())) {
			$mod_view ='v';
			$viewable = ($this->scheduleType != READ_ONLY || ($this->scheduleType == READ_ONLY && $conf['app']['readOnlyDetails']));
			$summary->visible = 0;
			$summaryhover->visible = 0;
			$viewable = false;
		}
...
// 2 reservation multiple resources - slave
		$clusterstatus=0; //default
		if ( $mod_view='m' ) {
			// janr	 part of cluster?
			$clusterstatus  = CmnFns::get_cluster_status ($rs['resid'], $rs['clusterid']); // cluster_status
				if ($clusterstatus > 0) {
					$mod_view='c'; // and clusterid is key
					write_reservation($colspan, $color_select, $mod_view, $rs['clusterid'], $summary, $summaryhover, $viewable, $this->scheduleType == READ_ONLY, $is_pending);
				} else {
					write_reservation($colspan, $color_select, $mod_view, $rs['resid'], $summary, $summaryhover, $viewable, $this->scheduleType == READ_ONLY, $is_pending);
				}
		} else {
			write_reservation($colspan, $color_select, $mod_view, $rs['resid'], $summary, $summaryhover, $viewable, $this->scheduleType == READ_ONLY, $is_pending);
		}
