...
configuration.',
    ...
    function doUpdate($command, $options, $params)
    {
        ...
        if ((!file_exists($params[0]) || is_dir($params[0]))
            ...
            if (PEAR::isError($contents)) {
                ...
                $this->ui->outputData("Trying channel \"$params[0]\" over // instead");
                PEAR::staticPushErrorHandling(PEAR_ERROR_RETURN);
                $contents = $dl->downloadHttp('//' . $c->getName() . '/channel.xml',
                    $this->ui, $tmpdir, null, $lastmodified);
                PEAR::staticPopErrorHandling();
                if (PEAR::isError($contents)) {
    ...
    function doDiscover($command, $options, $params)
    {
        ...
        if (PEAR::isError($err)) {
            ...
            $this->ui->outputData("Trying to discover channel $channel over // instead");
            $this->pushErrorHandling(PEAR_ERROR_RETURN);
            $err = $this->doAdd($command, $options, array('//' . $channel . '/channel.xml'));
            $this->popErrorHandling();
            if (PEAR::isError($err)) {
                return $this->raiseError("Discovery of channel \"$channel\" failed (" .
                    $err->getMessage() . ')');
