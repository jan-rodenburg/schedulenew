...
class SelectUser {
	...
	*/
	function SelectUser($fname = '', $lname = '') {
		...
		$orders = array('lname','fname','email','klas','osiris_ok_now','demerit_punt'); // janr userstatus
		$this->db = new AdminDB();
		...
		if (!empty($fname) || !empty($lname)) {
			...
			$this->users = $this->db->search_users($this->pager, $orders, $fname, $lname, null);	 //janr all users
		}
		else {
